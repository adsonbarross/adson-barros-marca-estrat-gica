...
export function ProjectsSection() {
  return (
    <section id="projetos" className="py-24 bg-foreground px-10">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <div>
            <h2 className="text-5xl font-bold text-background mb-3">Clientes reais</h2>
            <p className="text-background/60 max-w-md">Marcas que confiaram no meu trabalho de identidade visual.</p>
          </div>
          <a
            href={WHATSAPP_LINK}
            target="_blank"
            rel="noopener noreferrer"
            className="text-xs tracking-widest uppercase text-background/70 hover:text-orange transition-colors duration-300 whitespace-nowrap underline underline-offset-4"
          >
        ...
        <div className="grid grid-cols-4 gap-4 max-w-3xl mx-auto">
          {projects.map((project, index) => (
            ...
            >
              ...
              <p className="mt-2 text-center text-background/60 text-sm">{project.title}</p>
            </motion.div>
          ))}
        </div>
      </div>
